...
import RouteCreator from "../hooks/RouteCreator.jsx";

export default function MapPlaceholder() {
    const startPoints = Array.from({ length: 2 }, (_, i) => {
        const base = { lat: 52.0116, lng: 4.3571 };
        const latOffset = (Math.random() - 0.5) * 0.03;
        const lngOffset = (Math.random() - 0.5) * 0.02;
        return { id: `s${i + 1}`, lat: base.lat + latOffset, lng: base.lng + lngOffset, title: `start ${i + 1}` };
    });

    const endPoints = Array.from({ length: 2 }, (_, i) => {
        const base = { lat: 52.0116, lng: 4.3571 };
    ...
    return (
        <APIProvider apiKey={GOOGLE_MAPS_API_KEY} libraries={['marker', 'routes']}> {/* ✅ add 'routes' */}
            <Map
                defaultZoom={18}
                defaultCenter={{ lat: 52.010338, lng: 4.356907 }}
                mapId={MAP_ID}
